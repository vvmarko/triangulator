...
void SimpComp::save_complex_to_xml_file(const string& filename)
{
    ...
    UniqueIDColor::colorize_entire_complex(this);

    xml_document<> tree_xml;
    xml_node<>* base = tree_xml.allocate_node(node_element, this->name.c_str());
    tree_xml.append_node(base);

    vector<xml_node<>*> levels = this->get_element_levels_as_xml_nodes(tree_xml);
    for (auto nd : levels) base->append_node(nd);

    string nd_str;
    string color_type_str;
    string color_val_str;
    for (auto lvl : this->elements) {
        for (auto ks : lvl) {
            ...
            vector<xml_node<>*> neighbors = ks->neighbors->get_element_levels_as_xml_nodes(tree_xml);
...
vector<rapidxml::xml_node<>*> SimpComp::get_element_levels_as_xml_nodes(rapidxml::memory_pool<>& mp)
{
    using namespace rapidxml;
    ...
    for (int lvl = 0; lvl < this->elements.size(); lvl++) {
        // TODO: Memory leaking problems.
        ...
        simps_str = "";
        first = true;
        for (auto ksimp : this->elements[lvl]) {
            if (!first) {
                simps_str += ",";
            }
            else first = false;
